...
void CMemory::_initialize()
{
	stat_calls = 0;
}

void CMemory::_destroy()
{
}
...
{
	return mMemory.mem_alloc(size, static_cast<size_t>(alignment));
}
...
{
	mMemory.mem_free(ptr);
}

void operator delete(void* ptr, align_val_t alignment) noexcept
{
	mMemory.mem_free(ptr, static_cast<size_t>(alignment));
}
